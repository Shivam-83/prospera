...
const ChatBoostConfidence = () => {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${WS_BASE}/ws/tips?userID=${storedUserId}`;
    ...
    socket.onclose = (event) => {
      console.log('Tips WebSocket disconnected, code:', event.code, 'reason:', event.reason);
      setIsConnected(false);
      setIsLoading(false);
    };

    socket.onerror = (error) => {
      console.error('Tips WebSocket error:', error);
      setIsConnected(false);
      setIsLoading(false);
    };
