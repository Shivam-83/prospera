...
const ChatNegotiation = () => {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${WS_BASE}/ws/negotiation?userID=${storedUserId}`;
    ...
    socket.onclose = (event) => {
      console.log('Negotiation WebSocket disconnected, code:', event.code, 'reason:', event.reason);
      setIsConnected(false);
      setIsLoading(false);
    };

    socket.onerror = (error) => {
      console.error('Negotiation WebSocket error:', error);
      setIsConnected(false);
      setIsLoading(false);
    };
    ...
    <div className="chatbot-page">
      ...
      <div className="chat-container">
        <div className="chatbot-header">
          <h2 className="chat-title">Negotiation Coach</h2>
          <span style={{
            fontSize: '0.75rem',
